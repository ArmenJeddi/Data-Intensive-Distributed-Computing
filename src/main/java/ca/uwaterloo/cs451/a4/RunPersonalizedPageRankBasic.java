...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      numSources = conf.get("SOURCES_FIELD").split(",").length ;
      intermediateMass.setSources(sourceNodes) ;
      intermediateStructure.setSources(sourceNodes) ;
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      // if (nid.get() == 367 || nid.get() == 249 || nid.get() == 145 ){
      //   System.out.println("\n\n\n*******************************\nBefore") ;
      //   System.out.println("In mapper") ;
      //   System.out.println(Arrays.toString(node.getPageRankArray())) ; 
      //   System.out.println("\n\n\n*******************************\n") ;
      // }
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        for (int i=0; i < numSources; i++){
            mass[i] = node.getSourcePageRankByIndex(i) - (float) StrictMath.log(list.size());
        }
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          for (int k=0; k < numSources; k++){
            intermediateMass.setSourcePageRankByIndex(k, mass[k]);
          }
...
/*
  private static class MapWithInMapperCombiningClass extends
    ...
    private static class PageRankArray {
        float[] pagerank_val ;
        public PageRankArray(int numberOfSources){
            pagerank_val = new float[numberOfSources];
        }

        public void setArray(float[] ranks){
            for (int i=0; i < ranks.length ; i++){
                this.pagerank_val[i] = ranks[i] ;
            }
        }
        public void setByIndex(int index, float p){
            pagerank_val[index] = p ;
        }

        public float getByIndex(int index){
            return pagerank_val[index] ;
        }
    }

    private static final HMapIV<PageRankArray> map = new HMapIV<>();
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          if (map.containsKey(neighbor)) {
            ...
            massMessagesSaved++;
            PageRankArray prArray = map.get(neighbor) ;
            for (int k = 0; k < numSources; k++){
                prArray.setByIndex(k, sumLogProbs(mass[k], prArray.getByIndex(k)) ) ;
            }
            map.put(neighbor, prArray);
          } else {
            // New destination node; add new entry in map.
            massMessages++;
            PageRankArray prArray = new PageRankArray(numSources) ;
            prArray.setArray(mass) ;
            map.put(neighbor, prArray);
          }
        }
      }
    ...
    public void cleanup(Context context) throws IOException, InterruptedException {
      ...
      mass.setSources(sourceNodes) ;

      for (MapIV<PageRankArray>.Entry e : map.entrySet()) {
        k.set(e.getKey());

        PageRankArray prArray = e.getValue() ;

        mass.setNodeId(e.getKey());
        mass.setType(PageRankNode.Type.Mass);
        for (int k =0; k < numSources ; k++){
            mass.setSourcePageRankByIndex(k, prArray.getByIndex(k)) ;
        }
  ...
  */
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      while (values.hasNext()) {
        ...
        if (n.getType().equals(PageRankNode.Type.Structure)) {
          ...
          for (int i = 0; i < numSources; i++){
              mass[i] = sumLogProbs(mass[i], n.getSourcePageRankByIndex(i) );
          }
      ...
      for (int i = 0; i < numSources; i++){
        node.setSourcePageRankByIndex(i, mass[i]);
      }
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      missingMassStr = conf.get("MissingMassArray").replaceAll("[\\[\\]]","").replaceAll(" ", "").split(",");
      numSources = missingMassStr.length ;
      missingMass = new float[numSources] ;
      for (int k = 0; k < numSources; k++){
          missingMass[k] = Float.parseFloat(missingMassStr[k]) ;
      }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      // if (nid.get() == 367 || nid.get() == 249 || nid.get() == 145 ){
      //   System.out.println("\n\n\n*******************************\nBefore") ;
      //   System.out.println("In MapPageRankMassDistributionClass") ;
      //   System.out.println(Arrays.toString(node.getPageRankArray())) ; 
      //   System.out.println("\n\n\n*******************************\n") ;
      // }

      for (int i = 0; i < numSources; i++){
          float p = node.getSourcePageRankByIndex(i);

          if (nid.get() == sourceNodes[i]){
              float jump = (float)Math.log(ALPHA);
              float link = (float) Math.log(1.0f - ALPHA) + sumLogProbs(p, (float)Math.log(missingMass[i]));
              p = sumLogProbs(jump, link);
          }else{
            float jump = (float) Math.log(1.0f - ALPHA) ;
            p += jump ;
          }
          node.setSourcePageRankByIndex(i, p) ;
      }
  ...
  public int run(String[] args) throws Exception {
    ...
    int numSources = sourceList.split(",").length ;
    // Iterate PageRank.
    for (int i = s; i < e; i++) {
      iteratePageRank(i, i + 1, basePath, n, useCombiner, useInmapCombiner, sourceList);
    }
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
      boolean useCombiner, boolean useInMapperCombiner, String sourceList) throws Exception {
    // Each iteration consists of two phases (two MapReduce jobs).

    // Job 1: distribute PageRank mass along outgoing edges.
    float[] mass = phase1(i, j, basePath, numNodes, useCombiner, useInMapperCombiner, sourceList);
    ...
    phase2(i, j, missing, basePath, numNodes, sourceList);
  }
  ...
  private float[] phase1(int i, int j, String basePath, int numNodes,
    ...
    job.getConfiguration().set("SOURCES_FIELD", sourceList);
    ...
    */
    ...
    System.out.println("\n\n\n\n***************************************") ;
    System.out.printf("Iter %d is finished\n", i) ;
    System.out.println("\n\n\n\n***************************************") ;
    return mass;
  }

  private void phase2(int i, int j, float[] missing, String basePath, int numNodes, String sourceList) throws Exception {
    Job job = Job.getInstance(getConf());
    ...
    job.getConfiguration().setBoolean("mapred.map.tasks.speculative.execution", false);
    job.getConfiguration().setBoolean("mapred.reduce.tasks.speculative.execution", false);
    job.getConfiguration().set("MissingMassArray", Arrays.toString(missing));
    job.getConfiguration().setInt("NodeCount", numNodes);
